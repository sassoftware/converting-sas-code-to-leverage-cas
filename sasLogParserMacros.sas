...
*/ 
%put sasLogParserMacrosStoredProcessServer.sas version 3.14 07Oct2021:11:10;
...
%global noteStart noteStop;
filename saslog "&file";

/*START data drive method to set macro variables noteStart and noteStop */
data noteStartStop;
   ...
   product="&test"; 
   input line $100. ;
   noteStart=find(line,'NOTE: ');
   noteStop=100-noteStart;
   keep noteStart noteStop;
run;
proc freq data=noteStartSTop;
   table notestart / out=freqStart; 
run;
proc freq data=noteStartSTop; 
   table noteStop  / out=freqStop;
run;
proc sort data=freqStart ;
   by descending count;
run;
proc sort data=freqStop;
   by descending count;
run;

data noteStart;
   set freqStart (where=(noteStart ne 0));
   if _n_ = 1 then
	 call symput('noteStart',noteStart); 
run;
data noteStop;
   set freqStop (where=(noteStop ne 100));
   if _n_ = 1 then
	 call symput('noteStop',noteStop); 
run;
/*STOP data drive method to set macro variables noteStart and noteStop */
...
/*   stop:  Stored Process Server Log */
   ...
   if noteStart gt 1 then 
      put noteStart= noteStop= line=;
